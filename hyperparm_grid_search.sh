    ...
    for weight_decay in ${WEIGHT_DECAY[@]} ; do
        for rel_filter_threshold in ${REL_FILTER_THRESHOLD[@]} ; do
            for prop_drop in ${PROP_DROP[@]} ; do
                ...
                #python spert.py train --config configs/train_bert4L.conf --lr  $lr --weight_decay $weight_decay --rel_filter_threshold $rel_filter_threshold --prop_drop $prop_drop
                python spert.py train --config configs/train_roberta4L.conf --lr  $lr --weight_decay $weight_decay --rel_filter_threshold $rel_filter_threshold --prop_drop $prop_drop
            done 
        done 
    done 
done